 ...
 */
function cacheKey () {
    return `${this.cache.model.name}:select:${stableId(this.query.args)}`
}
...
function cacheQuery () {
    ...
    var maxRecordNumberPromise = this.cache.model.maxRecordNumber({
        ...
        // debug
        debug('select:initialMaxRecordNumber', n)
...
function cacheValid () {
    ...
    if (this.cached === null) {
        debug('select:cacheValid:false:null')
        return false
    }
    // false if n does not match
    if (this.cached.n !== this.initialMaxRecordNumber) {
        debug('select:cacheValid:false:maxRecordNumber')
        return false
    }
    // cache is valid
    debug('select:cacheValid:true')
    return true
}
