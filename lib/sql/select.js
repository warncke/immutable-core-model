...
function buildWhere (name, value, alias) {
    ...
    if (name.match(/\./)) {
        ...
        // if relation is direct
            ...
            // get param name
            var paramName = this.newParam(relation.relationIdColumn)
            // build sql
            whereSql = this.alias
                +'.'+this.quoteName(relation.relationIdColumn)
                +' = UNHEX(:'+paramName+')'
            // get id value which may be set with either full or default name
            var paramValue = value[relation.relationIdColumn] || value[relation.model.defaultColumns[relation.relationIdColumn]]
            // add value to params
            this.params[paramName] = paramValue
        }
