...
function buildWhere (name, value, alias) {
    return name === 'relation'
        ? this.buildWhereRelation(name, value, alias)
 ...
 * @throws {Error}
 */
function buildWhereRelation (name, value, alias) {
...
function checkSelectById () {
    ...
    // id must be string or array
    if (typeof this.args.where.id !== 'string' && !Array.isArray(this.args.where.id)) {
        return
    }
    ...
    // select by id on models using joined access models not supported
    if (defined(this.model.accessModel)) {
        return
    }
...
function selectByIdWhereSql () {
    ...
    if (Array.isArray(this.args.where.id)) {
        ...
        var placeholders = _.map(this.args.where.id, id => {
            ...
            // create placeholder
            return `UNHEX(:${paramName})`
        })
        // return where sql
        return `WHERE ${columnSql} IN(${placeholders.join(', ')})`
    }
    // otherwise build equals query
        ...
        // return where sql
        return `WHERE ${columnSql} = UNHEX(:id)`
    }
}
