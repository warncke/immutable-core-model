...
function buildWhere (name, value, alias) {
    ...
    if (name.match(/\./)) {
        ...
        if (relation.via) {
            ...
            // join relation table to via table
            this.join.push( this.joinSql(relationModel.name, relationAlias, relation.relationIdColumn, viaAlias, relation.viaRelationIdColumn) )
            // get param name
        ...
        // relation is direct
            ...
            // join related table
            this.join.push( this.joinSql(relationModel.name, relationAlias, relation.relationIdColumn, this.alias, relation.modelIdColumn) )
            // get param name
            var paramName = this.newParam(relation.relationIdColumn)
            // build sql
            whereSql = relationAlias
