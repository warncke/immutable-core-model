...
function buildWhere (name, value, alias) {
    ...
    if (name.match(/\./)) {
        ...
        if (relation.via) {
            ...
            // relation is joined on other meta column (sessionId, accountId)
            else if (viaModel.columnName(relationIdColumn)) {
                // add join
                this.join.push( this.joinSql(relationModel.name, relationAlias, relationIdColumn, viaAlias, viaModel.columnName(relationIdColumn)) )
                // get param name
                var paramName = this.newParam(relationIdColumn)
                // build sql
                whereSql = relationAlias
        ...
        // relation is direct
            ...
            // relation is joined on other meta column (sessionId, accountId)
            else if (this.model.columnName(relationIdColumn)) {
                // add join
                this.join.push( this.joinSql(relationModel.name, relationAlias, relationIdColumn, this.alias, this.model.columnName(relationIdColumn)) )
                // get param name
                var paramName = this.newParam(relationIdColumn)
                // build sql
                whereSql = relationAlias
