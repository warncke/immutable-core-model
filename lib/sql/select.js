...
function buildOrderArray (order) {
    ...
    this.order.push(
        _.map(
            order, column => this.model.columnName(column, true)
        ).join(', ')+' '+direction
    )
}
 ...
 * @throws {Error}
 */
function buildWhere (name, value) {
    ...
    if (typeof value === 'string' || typeof value === 'number') {
        ...
        var whereSql = spec.type === 'id'
            ...
            ? this.getAlias(model.name)+'.'+name+' = UNHEX(:'+paramName+')'
            : this.getAlias(model.name)+'.'+name+' = :'+paramName
    ...
    // if value is array then do IN () query
        ...
        // create sql
        var whereSql = this.getAlias(model.name)+'.'+name+' IN('
            +placeholders.join(', ')+')'
        // add sql to list of where clauses
        this.where.push(whereSql)
    }
 ...
 * @throws {Error}
 */
function leftJoinSql (tableA, aliasA, column, aliasB) {
    return 'LEFT JOIN '+this.quoteName(tableA)+' '+aliasA
        +' ON '+aliasA+'.'+this.quoteName(column)+' = '
        +aliasB+'.'+this.quoteName(column)
}
