...
SqlSelect.prototype = {
    ...
    addAccessControlLocal: addAccessControlLocal,
    addAccessControlModel: addAccessControlModel,
    addAction: addAction,
...
function addAccessControl () {
    ...
    // add access control clause to query
    defined(this.args.accessModel)
        // access id column is on linked table
        ? this.addAccessControlModel()
        // access id column is on current model
        : this.addAccessControlLocal()
}

/**
 * @function addAccessControlLocal
 *
 * add access id column from current model to where clause
 *
 * @throws {Error}
 */
function addAccessControlLocal () {
    ...
    // add where SQL
    this.where.push(`${alias}.${column} = UNHEX(:${paramName})`)   
}

/**
 * @function addAccessControlModel
 *
 * add access id column from related model to where clause
 *
 * @throws {Error}
 */
function addAccessControlModel () {
    // get access control model - throws error if not found
    var accessModel = this.model.getModel(this.args.accessModel)
    // access model must have access id column
    if (!accessModel.columns[this.args.accessIdName]) {
        throw new Error(`accessIdName ${this.args.accessIdName} not defined on accessModel ${accessModel.name} for ${this.model.name}`)
    }
    // name of column to join on
    var joinColumn
    // use id column
    if (this.model.defaultColumnsInverse.id && accessModel.columns[this.model.defaultColumnsInverse.id]) {
        joinColumn = this.model.defaultColumnsInverse.id
    }
    // use original id column
    else if (this.model.defaultColumnsInverse.originalId && accessModel.columns[this.model.defaultColumnsInverse.originalId]) {
        joinColumn = this.model.defaultColumnsInverse.originalId
    }
    // no joinable column
    else {
        throw new Error(`could not join accessModel ${accessModel.name} for ${this.model.name}`)
    }
    // create alias for relation table to use in query
    var accessAlias = this.newAlias(accessModel.name)
    // add join
    this.join.push(this.joinSql(accessModel.name, accessAlias, joinColumn, this.alias))
    // if access record can be deleted must also join on delete record so
    // that deleted access records are not counted
    if (accessModel.actions.delete) {
        var actionModel = accessModel.actions.delete.model
        // get table name for action model
        var actionTable = actionModel.name
        // get alias for table
        var actionAlias = this.getAlias(actionTable)
        // get id column for action table
        var actionIdColumn = actionModel.columnName('id')
        // get id column for primary table
        var primaryIdColumn = accessModel.columnName('id')
        // add left join for action table to primary table
        this.join.push( this.leftJoinSql(actionTable, actionAlias, primaryIdColumn, accessAlias) )
        // get quoted column name
        actionIdColumn = this.quoteName(actionIdColumn)
        // add where sql to skip any columns where delete is not null
        this.where.push(`${actionAlias}.${actionIdColumn} IS NULL`)
    }
    // validate column name - throw error
    var column = accessModel.columnName(this.args.accessIdName, true)
    // get column spec
    var spec = accessModel.columns[column]
    // add select column sql
    this.select.push(
        this.selectColumnSql(accessModel, column, spec, accessAlias)
    )
    // get quoted column name
    column = this.quoteName(column)
    // get param name for value placeholder
    ...
    this.params[paramName] = this.args.accessId
    // add where sql
    this.where.push(`${accessAlias}.${column} = UNHEX(:${paramName})`)
}
...
function addAction (action, name) {
    ...
    if (!this.onlyIds) {
        ...
        _.each(action.model.columns, (spec, column) => {
            ...
            this.select.push( this.selectColumnSql(this.model, column, spec, actionTable) )
        })
    }
    ...
    if (!this.onlyIds) {
        ...
        _.each(action.inverse.columns, (spec, column) => {
            ...
            this.select.push( this.selectColumnSql(this.model, column, spec, inverseTable) )
        })
    }
}
...
function buildRevision () {
    ...
    if (this.args.where.id || this.args.where[primaryIdColumn]) {
        ...
        if (this.args.current) {
            ...
            this.select.push(`LOWER(HEX(${originalAlias}.${this.quoteName(primaryIdColumn)})) AS ${this.quoteName(primaryIdColumnSelect)}`)
        }
        ...
        // parentId = id to check if record is current
            ...
            this.select.push(
                'LOWER(HEX('+revisionAlias+'.'+this.quoteName(primaryIdColumn)+')) AS childId'
            )
        }
    }
...
function buildSelect () {
    ...
    if (this.onlyIds) {
        ...
        // otherwise select all
            ...
            // if joining any models then add columns from those models
            _.each(this.joinRelationNames, relationName => {
                // get relation
                var relation = this.model.relation(relationName)
                // add relation columns
                _.each(relation.model.columnNames, columnName => {
                    // add to columns with relation as prefix
                    columns.push(relationName+'.'+columnName)
                })
            })
        }
    }
    ...
    _.each(columns, columnName => {
        // split on dot to check if column has prefix
        var parts = columnName.split('.')
        // column does not have prefix
        if (parts.length === 1) {
            // validate column name - throw error
        ...
        // column has prefix
            ...
            var prefix = parts[0]
            columnName = parts[1]
            // prefix must be for model being joined on
            assert.ok(this.joinRelations[prefix], 'invalid prefix '+prefix+' for column in select - model not joined')
            // get join to get prefix for table
            var join = this.joinRelations[prefix]
            // get relation to get related model
            var relation = this.model.relation(prefix)
            // validate column name - throw error
            columnName = relation.model.columnName(columnName, true)
            // get spec
            var spec = relation.model.columns[columnName]
            // add select column sql
            this.select.push(
                this.selectColumnSql(relation.model, columnName, spec, join.relationAlias)
            )
        }
        // error
        else {
            throw new Error('invalid column name in select '+columnName)
        }
    })
}
...
function buildWhere (name, value, alias) {
    ...
    if (name.match(/\./)) {
        ...
        // prefix is for an action
        if (this.model.actions[prefix]) {
            // get action object
            var action = this.model.actions[prefix]
            // use action model
            model = action.model
        ...
        // prefix is for join
        else if (this.joinRelations[prefix]) {
            // get join
            var join = this.joinRelations[prefix]
            // get relation
            var relation = this.model.relation(prefix)
        ...
        // build sql
        var whereSql
        // if relation is via a linking table then join that table and select
        // rows that match the relation id being queried
        if (relation.via) {
            // get via model
            ...
            // build sql
            whereSql = viaAlias
                +'.'+this.quoteName(relation.viaRelationIdColumn)
                +' = UNHEX(:'+paramName+')'
            // get id value which may be set with either full or default name
            var paramValue = value[relation.viaRelationIdColumn] || value[relation.model.defaultColumns[relation.viaRelationIdColumn]]
            // add value to params
            this.params[paramName] = paramValue
        }
        // if relation is direct
        else {
            // get param name
            var paramName = this.newParam(relation.modelIdColumn)
            // build sql
            whereSql = this.alias
                +'.'+this.quoteName(relation.modelIdColumn)
                +' = UNHEX(:'+paramName+')'
            // get id value which may be set with either full or default name
            var paramValue = value[relation.modelIdColumn] || value[relation.model.defaultColumns[relation.modelIdColumn]]
            // add value to params
            this.params[paramName] = paramValue
        }
