...
function initActions (args) {
    ...
    _.each(args.actions, (spec, name) => {
        ...
        // set name for method that checks if action has been performed
        if (!action.isMethod) {
            // default is: isActioned e.g. isDeleted isPublished
            action.isMethod = 'is'+changeCase.pascalCase(name)
                // if name ends in e then add d otherwise add ed
                + (name.match(/e$/) ? 'd' : 'ed')
        }
        // set name used in queries to checks if action has been performed
        if (!action.whereName) {
            // if name ends in e then add d otherwise add ed
            action.whereName = name + (name.match(/e$/) ? 'd' : 'ed')
        }
        ...
        action.model = this.new(action.model)
        ...
        if (action.inverse) {
            ...
            if (!action.inverse.name) {
                ...
                // e.g. fooUnDelete
                action.inverse.name = this.name+'Un'+changeCase.pascalCase(name)
            }
            ...
            action.inverse = this.new(action.inverse)
        }
