...
function alterColumn (args) {
    ...
    if (modelSpec.type === schemaSpec.type && modelSpec.index === schemaSpec.index && modelSpec.unique === schemaSpec.unique) {
        return
    }
 ...
 */
function createElasticsearchIndex () {
    // do nothing if model does not have or require client
    ...
    // create index if it doesnt exists
    .then(exists => {
        if (!exists) {
...
function sync (args) {
    ...
    var elasticSearchIndexPromise = this.createElasticsearchIndex()
    ...
    // wait for elasticsearch index creation
    .then(() => elasticSearchIndexPromise)
}
