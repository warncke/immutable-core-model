...
function initModule (args) {
    ...
    this.title = changeCase.capitalCase(this.name)
    ...
    // if opensearch is true then client will be set later and is required
    if (args.opensearch === true) {
        this.opensearchClient = true
    }
    // set opensearch client
    else if (args.opensearch) {
        this.opensearch(args.opensearch)
    }
    else {
    ...
    this.osIndex = args.osIndex || this.path
