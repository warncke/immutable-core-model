...
function initColumns (args) {
    ...
    _.each(defaultColumns, (spec, defaultName) => {
        // prefix all columns besides n/c with model name
        var name = defaultName === 'c' || defaultName === 'n'
            ? defaultName
    ...
    // get column names without c/n which go first
    var columnNames = _.filter(_.keys(this.columns), columnName => {
        return columnName === 'c' || columnName === 'n' ? false : true
    }).sort()
    // if there is c column add first
    if (defined(this.columns.c)) {
        columnNames.unshift('c')
    }
    // if there is n column add first
    if (defined(this.columns.n)) {
        columnNames.unshift('n')
    }
    // create sorted list of column names for generating queries
    this.columnNames = columnNames
    // get list of column names to insert
    this.insertColumnNames = _.filter(this.columnNames, columnName => {
        return columnName === 'n' ? false : true
    })
