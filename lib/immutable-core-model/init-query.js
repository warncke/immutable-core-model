...
function query (model, args) {
    ...
    if (!args.allow) {
        ...
        // if scope is own then add access id to where clause
            ...
            // create where clause if not defined
            if (!args.where) {
                args.where = {}
            }
...
function result (model, args, res, select) {
    ...
    // if query is for all records then return all
        ...
        var records = _.map(res, raw => instanceFromRes(model, raw, args.session, args.raw, select, args.allow))
        // if query is for multiple records by id then re-order records in the
        // order of the ids that were queried
        if (records.length > 1 && args.where && Array.isArray(args.where.id)) {
            // build map of records by id
            var recordsById = _.keyBy(records, 'id')
            // re-order records in order of where id query
            records = _.map(args.where.id, id => {
