...
function query (model, args) {
    ...
    // if this is request for a single record by id then it is a "read"
    var isRead = args.where && args.where.id && args.limit === 1
    ...
    // if scope is own then add access id to where clause
        ...
        var accessId = args.session && args.session[model.accessIdName]
        // throw access denied error if no accessId
        if (!accessId) {
            httpError(403)
        }
        ...
        // add accessId to where clause
        args.where[model.accessIdName] = accessId
    }
    // get select sql
