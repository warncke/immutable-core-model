    ...
    if (!args.allow) {
        ...
        // if scope is own then add access id to where clause
            ...
            if (defined(session.accessId) && session.accessIdName === model.accessIdName) {
                ...
                if (model.accessModel) {
                    args.accessModel = model.accessModel
                }
                // add access id to args
                args.accessIdName = session.accessIdName
                args.accessId = session.accessId
            }
            // otherwise try account id
            else if (defined(session.accountId) && model.defaultColumnsInverse.accountId) {
                // add access id to args
                args.accessIdName = 'accountId'
                args.accessId = session.accountId
            }
