...
async function query (model, args) {
    ...
    .then(res => {
        ...
        if (modelViews && modelViews.length) {
            ...
            return modelViewEngine.result(res)
        }
        // otherwise create result based on args
        else {
            return result(model, args, res)
        }
    })
}
...
function getModelViewsForQuery (model, args) {
    ...
    if (args.views !== undefined) {
        ...
        // otherwise views must be array
        assert.ok(Array.isArray(args.views), 'query views must be array')
        // use args views
        views = args.views
    }
 ...
 * @throws {Error}
 */
function result (model, args, res) {
