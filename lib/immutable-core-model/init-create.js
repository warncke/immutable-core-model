...
function initCreate (args) {
    ...
    this.createMeta = immutable.method(methodSignature, function (args) {
        return createMeta(model, args)
    }, {
...
function createMeta (model, args) {
    ...
    _.each(model.extraColumns, (spec, name) => {
        ...
        if (spec.type === 'boolean') {
            raw[name] = sql.toBooleanInt(raw[name])
        }
    })
    ...
    // attempt insert
    var promise = model.mysql().query(insertSql, raw)
    // do secondary storage (opensearch, redis) on insert success
    .then((res) => {
    ...
    // wait for response
        ...
        .catch(err => {
            ...
            if (args.duplicate && err.errno === 1062) {
                //  do not return response if flag set
                if (args.response === false) {
                    return
                }
