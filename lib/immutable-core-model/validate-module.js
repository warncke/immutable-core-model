...
async function validateData (args) {
    ...
    await res.each(async (rec, num) => {
        ...
        // do update
        await this.mysql().query(update.sql, update.params, {}, args.session)
...
async function validateDeleted (args) {
    try {
        var deletedTable = await this.mysql().query(
    ...
    // get deleted records
    const [ results ] = await this.mysql().query(sql.selectDeleted(this))
    debug(`checking ${results.length} deleted records`)
