...
function ImmutableCoreModelResult (args) {
    ...
    // get id column name
    var idColumn = args.model.defaultColumnsInverse.id
    // require id column
    assert(defined(idColumn), `${args.model.name} result error: id column required`)
    // get ids from raw result - array of arrays having id as element
    this.ids = _.map(args.raw, row => row[idColumn])
