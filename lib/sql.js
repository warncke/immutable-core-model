...
function insert (model) {
    return 'INSERT INTO '+quoteName(model.name)
        ...
        +_.map(
            model.columnNames,
            columnName => placeholder(columnName, model.columns[columnName])
        ).join(', ')
        +')'
}
 ...
 * @throws {Error}
 */
function placeholder (name, spec) {
...
function select (model, args) {
    ...
    columns = _.map(columns, columnName => {
        ...
        return spec.type === 'id'
            ...
            ? 'HEX('+primaryAlias+'.'+quoteName(columnName)+') as '+quoteName(columnName)
            // use plain column name
            : primaryAlias+'.'+quoteName(columnName)
    })
