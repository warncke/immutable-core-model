...
function select (model, args) {
    ...
    var primaryAlias = selectTableAlias(aliases, primaryTable)
    var primaryIdColumn = model.columnName('id', true)
    // list of join statements for query
    var join = []
    // list of condition statements for select
    var where = []
    // list of columns to select
    var columns
    // flag set to try if only ids selected
    var onlyIds = false
    // if this is not a select all or select one then only select the ids
    // that match the query which will be returned with a result iterator
    // that will fetch the actual data
    if (!args.all && args.limit !== 1) {
        columns = ['id']
        // if only selecting ids then use array response
        options.useArray = true
        // only ids being selected - do not select action columns
        onlyIds = true
    }
    // otherwise select either all columns or the specific columns specified
    // in select for query
    else {
        // if specific list of columns was passed the select those
        if (Array.isArray(args.select) && args.select.length) {
            columns = args.select
        }
    ...
    columns = _.map(columns, columnName => {
        ...
        return selectColumn(model, columnName, spec, primaryAlias)
    })
    // add actions to query
    _.each(model.actions, (action, name) => {
        // get table name for action model
        var actionTable = action.model.name
        // get alias for table
        var actionAlias = selectTableAlias(aliases, actionTable)
        // get id column for action table
        var actionIdColumn = action.model.columnName('id')
        // add left join for action table
        join.push(
            'LEFT JOIN '+quoteName(actionTable)+' '+actionAlias
            +' ON '+actionAlias+'.'+primaryIdColumn+' = '
            +primaryAlias+'.'+primaryIdColumn
        )
        // select action columns unless doing an id only query
        if (!onlyIds) {
            // add all action columns to query
            _.each(action.model.columns, (spec, name) => {
                // skip id column
                if (name === primaryIdColumn) {
                    return
                }
                // add column to select list
                columns.push(selectColumn(model, name, spec, actionAlias))
            })
        }
        ...
        var inverseAlias = selectTableAlias(aliases, inverseTable)
        // add left join for inverse table
        join.push(
            'LEFT JOIN '+quoteName(inverseTable)+' '+inverseAlias
            +' ON '+inverseAlias+'.'+actionIdColumn+' = '
            +actionAlias+'.'+actionIdColumn
        )
        // select action columns unless doing an id only query
        if (!onlyIds) {
            // add all action columns to query
            _.each(action.inverse.columns, (spec, name) => {
                // skip id column
                if (name === actionIdColumn) {
                    return
                }
                // add column to select list
                columns.push(selectColumn(model, name, spec, inverseAlias))
            })
        }
    })
    ...
    _.each(args.where, (value, columnName) => {
        ...
        // if value is array then do IN () query
            ...
            // create sql
            var whereSql = primaryAlias+'.'+columnName+' IN('
                +placeholders.join(', ')+')'
            // add sql to list of where clauses
            where.push(whereSql)
        }
 ...
 * @returns {string}
 */
function selectColumn (model, name, spec, table) {
    // encode binary column id values as hex
    if (spec.type === 'id') {
        return 'HEX('+table+'.'+quoteName(name)+') as '+quoteName(name)
    }
    // if compression is enabled then get data columns as hex
    else if (spec.type === 'data' && model.compression) {
        return 'TO_BASE64('+table+'.'+quoteName(name)+') as '+quoteName(name)
    }
    // use plain column name
    else {
        return table+'.'+quoteName(name)
    }
}
...
/**
 * @function selectTableAlias
 *
 * get table alias - adds entry to aliases object
 *
 ...
 * @returns {string}
 */
function selectTableAlias (aliases, tableName) {
    ...
    // if alias is not in use then use if
    if (!aliases[alias]) {
    ...
    // if alias is in use then append digit to it to get unique alias
    else {
        var i = 0
        ...
        while (i < 99) {
            ...
            if (!aliases[uniqueAlias]) {
                // add alias to map of aliases
                aliases[uniqueAlias] = tableName
                // return alias
                return uniqueAlias
            }
            // try next number
