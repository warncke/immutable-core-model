...
function selectDeleted (model) {
    ...
    // build sql
    return `SELECT LOWER(HEX(${quoteName(idColumn)})) AS id FROM ${model.name}Delete`
}
...
function updateExtraColumns (model, instance) {
    ...
    // build sql string
    var sql = `UPDATE ${quoteName(model.name)} SET ${updateColumns.join(', ')} WHERE ${quoteName(model.columnName('id'))} = UNHEX(:id)`
