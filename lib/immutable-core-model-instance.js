...
function action (name, args) {
    ...
    if (defined(accessIdName)) {
        // get value from record
        if (defined(this[accessIdName])) {
            accessId = this[accessIdName]
        }
        // get value from record data
        else if (defined(this.data) && defined(this.data[accessIdName])) {
...
function init (args) {
    ...
    // if accessId column is on another model then add to properties
    if (defined(this.model.accessModel) && defined(this.model.accessIdName)) {
        // set property
        this[this.model.accessIdName] = this.raw[this.model.accessIdName]
        // add property name to list of record properties
        this.properties.push(this.model.accessIdName)
    }
    ...
    if (defined(this.model.columnName('data'))) {
        ...
        _.each(this.model.extraColumns, (spec, columnName) => {
            if (!defined(_.get(this.data, spec.path))) {
