...
function inspect (depth, options) {
    ...
    var obj = _.omit(this, ['action', 'current', 'empty', 'init', 'inspect', 'model', 'promise', 'session', 'toJSON', 'update', 'updateMeta'])
...
function updateMeta (args) {
    ...
    if (args.data !== null) {
        ...
        _.each(this.model.extraColumns, (spec, name) => {
            ...
            if (spec.immutable && args.data[name] !== this.data[name]) {
                throw new Error('[immutable.model.'+this.model.name+'] cannot modify immutable property '+name)
            }
    ...
    .catch(err => {
        ...
        // if force is set but error is not a duplicate key error or error is
        // not on parentId then throw error
        if (err.code !== 1062 || err.message.indexOf(this.model.columnName('parentId')) === -1) {
            throw err
        }
