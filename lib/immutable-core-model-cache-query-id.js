...
function cacheQuery () {
    ...
    .then(cached => {
        ...
        for (var i=0; i < this.numIds; i++) {
            var result = cached[i]
            // if result is null add to list of uncached ids
            if (cached[i] === null) {
                this.uncachedIds.push(this.ids[i])
            }
            // otherwise and to cached list
            else {
                // decode JSON
                result = JSON.parse(result)
                // add cached flag
                result._cached = true
                // add decoded result to list of cached records
                this.cached.push(result)
            }
        }
...
function cacheResults (results) {
    ...
    _.each(results, result => {
        setArgs.push(this.cacheKey(result[idColumn]), JSON.stringify(result))
    })
    // cache results
