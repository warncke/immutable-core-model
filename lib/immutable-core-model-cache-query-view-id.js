...
function cacheQuery () {
    // debug
    debug('view:id:get', this.idCacheKeys, this.ids)
...
function cacheQueryResult (cached) {
    ...
    // debug
    debug('view:id:uncachedIds', this.uncachedIds)
 ...
 *
 * @param {ImmutableCoreModelViewEngine} modelViewEngine
 * @param {array} origResults
 * @param {array} viewResults
 *
 * @returns {Promise}
 */
function cacheResults (modelViewEngine, origResults, viewResults) {
    // get id column name
    var idColumn = this.query.model.columnName('id')
    // arguments for multi-set
    var setArgs = []
    // args are list of key, value, key, value
    _.each(viewResults, result => {
