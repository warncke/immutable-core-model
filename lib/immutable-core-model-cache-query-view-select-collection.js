...
function cacheQuery () {
    ...
    // get cached value
    var cacheGetPromise = this.cache.redis.getAsync(this.selectCacheKey)
    // store cached value
    .then(res => {
        // skip if not cached
        if (res === null) {
    ...
    var maxRecordNumberPromise = this.cache.model.maxRecordNumber({
        ...
        // debug
        debug('view:select:collection:initialMaxRecordNumber', n)
...
function cacheResults (results) {
    ...
    return this.cache.model.maxRecordNumber({
        ...
        if (cacheValue) {
            ...
            return this.cache.redis.setAsync(this.selectCacheKey, cacheValue)
        }
    })
}
...
function cacheValid () {
    ...
    if (this.cached === null) {
        debug('view:select:collection:cacheValid:false:null')
        return false
    }
    // false if n does not match
    if (this.cached.n !== this.initialMaxRecordNumber) {
        debug('view:select:collection:cacheValid:false:maxRecordNumber')
        return false
    }
    // cache is valid
    debug('view:select:collection:cacheValid:true')
    return true
}
